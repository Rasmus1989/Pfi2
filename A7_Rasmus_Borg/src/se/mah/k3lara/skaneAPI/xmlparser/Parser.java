	...
	public static Lines getStationResults(Station station){
		...
		if (xml!=null){
			...
			for (int i = 0; i < nl.getLength(); i++) { //Iterate all Line elements
				...
				//Linjenamn
				lineName = parser.getValue(e, "Name");
				// isTimingPoint true or false (whatever isTimingPoint is)
				String isTimingPointIn = parser.getValue(e, "IsTimingPoint");
				isTimingPoint = Boolean.parseBoolean(isTimingPointIn);
				// stoppoint (från hållplats);
				stopPoint = parser.getValue(e, "StopPoint");
				// Linjetyp id (converting value to int)
				String lineTypeIdIn = parser.getValue(e, "LineTypeId");
				lineTypeId=Integer.parseInt(lineTypeIdIn);
				// linjetyp
				...
				//....
				if(debug){System.out.println("Line name: " +lineName);}
